	...
	public static Uid createAccount(Set<Attribute> arg1, File targetFile, String uidAttribute) {
		...
		try {
			for (Attribute thisAttribute : arg1) {
				System.out.println(" ### Attribute Name" + thisAttribute.getName() + " Value "
		...
		System.out.println(
				" #### Found UID Attribute" + thisAttribute.getName() + " " + thisAttribute.getValue() + " #### ");
				...
				if (!attributeFound) {
					...
					for (Attribute attr : updatedAttributes) {
						...
						if (!line.contains(attr.getValue().toString()) && !line.contains(attr.getName().toString())) {
							//
							updatedLine.append(attr.getName()).append(":")
									.append(attr.getValue().get(attr.getValue().size() - 1)).append(",");
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				while (sc.hasNext()) {
					...
					if (strArray[strArray.length].equalsIgnoreCase(uidValue)) {
						accountExists = true;
						break;
					}
				}
